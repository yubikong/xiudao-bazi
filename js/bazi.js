...
(function () {
  ...
  // 近期流年（当前年份前5年~后5年，含今年）
  function bzRecentYears() {
    const GAN = '甲乙丙丁戊己庚辛壬癸', ZHI = '子丑寅卯辰巳午未申酉戌亥';
    function ygz(year) { return GAN[(year - 4) % 10] + ZHI[((year - 4) % 12 + 12) % 12]; }
    const curYear = _w.now.solar.getYear();
    const list = [];
    for (let y = curYear - 5; y <= curYear + 5; y++) list.push(y + ygz(y) + (y === curYear ? '(今年)' : ''));
    return list.join('  ');
  }
  function bzYun() {
  ...
  var BZ_AI_PROMPT =
    `你是深谙子平命理、师承梁湘润体系的资深命理师，精通《子平母法总则》《子平八字大突破》《女命通会详解》《大流年判例》。以下盘面数据已由程序完整排定：四柱、每柱藏干（含十神，已按日主换好）、每柱天干十神、旺衰、神煞、纳音、日主空亡、大运（含起运岁，标※当前者）、【当前】年份与虚岁、当前大运起止、【近期流年】干支均精确无误。你不得自行排盘、不得心算重排藏干十神或旺衰、不得自拟神煞、不得另推流年干支，只许直接采用盘面数据论命。请以"推演+论证"方式输出一份完整、详实、结构化的命理分析报告。
...
- 当前大运专项（按盘面【当前】【近期流年】段，必须重点展开）：先总评当前大运（标"※当前"者）之吉凶基调与关键应期，再以当前公历年份为中心，逐流年分析【近期流年】所列各年（前5年+今年+后5年）之运势；判断须落到"某年（干支，虚岁XX）发生何事、方向（成/散、吉/凶、得/失）"，依据按第三部分体例括注；过往5年作简述复盘，未来5年逐流年详断。
...
- 盘面数据（藏干十神/旺衰/神煞/大运/当前大运/近期流年）已由程序排定，凡自行重排、心算、自拟者断语作废；六亲星定位直接读盘面十神（偏财=父、正印=母），不得把劫财/比肩/官杀误作印星或父星。
...
1. 藏干（含十神，按日主换好）、天干十神、旺衰、神煞、纳音、空亡、大运、当前年份与虚岁、当前大运、近期流年均以盘面为准，不得心算重排或自拟。
